...
class _HTTPConnection(httputil.HTTPMessageDelegate):  # pylint: disable=too-many-instance-attributes
    ...
    async def run(self) -> None:
        try:
            ...
            host, port = self._parse_and_validate_url()

            # Configure socket options
            af, source_ip = self._configure_socket_options()

            # Get SSL options
            ssl_options = self._get_ssl_options(self.parsed.scheme)

            # Schedule connection timeout
            self._schedule_connect_timeout()
            ...
            # Verify connection wasn't timed out
            if self.final_callback is None:
                # final_callback is cleared if we've hit our timeout.
                stream.close()
                return

            # Set up stream and request timeout
            self.stream = stream
            self.stream.set_close_callback(self.on_connection_close)
            self._remove_timeout()
            ...
            self._schedule_request_timeout()

            # Validate HTTP method and proxy settings
            self._validate_http_method_and_proxy()
    ...
    def _parse_and_validate_url(self) -> tuple[str, int]:
        """Parse and validate the request URL.
        
        Extracts hostname and port from the parsed URL, validates the scheme,
        and handles IPv6 addresses.
    ...
    def _configure_socket_options(self) -> tuple[int, Optional[str]]:
        """Configure socket options (address family, source IP).
        
        Determines whether to use IPv4 or IPv6, and validates the network
        interface if specified.
        
        Returns:
            Tuple of (address_family, source_ip)
            
        Raises:
            ValueError: If network_interface is not a valid IP address.
        """
        if self.request.allow_ipv6 is False:
            af = socket.AF_INET
        else:
    ...
    def _schedule_connect_timeout(self) -> None:
        """Calculate and schedule the connection timeout.
        
        Determines the effective timeout (minimum of connect_timeout and
        request_timeout if both are set) and schedules a timeout callback.
        """
        if self.request.connect_timeout and self.request.request_timeout:
            timeout = min(
                self.request.connect_timeout, self.request.request_timeout
    ...
    def _schedule_request_timeout(self) -> None:
        """Schedule the request timeout (after connection is established).
        
        Sets a timeout for the request phase if request_timeout is configured.
        """
        if self.request.request_timeout:
            self._timeout = self.io_loop.add_timeout(
                self.start_time + self.request.request_timeout,
    ...
    def _validate_http_method_and_proxy(self) -> None:
        """Validate HTTP method and ensure no proxy settings are configured.
        
        Raises:
            KeyError: If HTTP method is not supported and allow_nonstandard_methods is False.
            NotImplementedError: If any proxy settings are configured (not supported).
        """
        if (
            self.request.method not in self._SUPPORTED_METHODS
            and not self.request.allow_nonstandard_methods
    ...
    def _validate_request_body(self) -> None:
        """Validate that request body presence matches HTTP method semantics.
        
        POST, PATCH, and PUT methods should have a body, while other methods
        should not (unless allow_nonstandard_methods is True).
        
        Raises:
            ValueError: If body presence doesn't match method semantics.
        """
        if not self.request.allow_nonstandard_methods:
            # Some HTTP methods nearly always have bodies while others
            # almost never do. Fail in this case unless the user has
    ...
    def _set_basic_headers(self) -> None:
        """Set basic request headers (Connection and Host).
        
        These headers are set for all requests and establish the basic
    ...
    def _set_authentication_and_user_agent(self) -> None:
        """Set authentication and User-Agent headers.
        
        Handles Basic authentication (from URL or request properties)
        and User-Agent header (custom or Tornado default).
        """
    ...
    def _set_body_and_encoding_headers(self) -> None:
        """Set headers related to request body and content encoding.
        
        Handles:
        - Expect header (100-continue if configured)
        - Content-Length header (if body is present)
        - Content-Type header (POST form encoding default)
        - Accept-Encoding header (gzip if decompression enabled)
        """
        ...
        # Content-Type header (POST default)
        if (
            self.request.method == "POST"
            and "Content-Type" not in self.request.headers
        ):
    ...
    def _set_request_headers(self) -> None:
        """Set default and derived headers for the HTTP request.
        
        Orchestrates setting all request headers by calling the
        appropriate helper methods.
        """
        self._set_basic_headers()
        self._set_authentication_and_user_agent()
        self._set_body_and_encoding_headers()
