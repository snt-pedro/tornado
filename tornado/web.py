...
class StaticFileHandler(RequestHandler):
    ...
    **Subclassing notes**
    ...
    def _handle_range_request(
        ...
        """Handle HTTP Range request header parsing and validation.

        Returns a tuple of (start, end, status_code) where:
    ...
    def _calculate_content_length(
        ...
        """Calculate the Content-Length header value based on range parameters."""
    ...
    async def _write_content_body(
        self, start: int | None, end: int | None
    ) -> None:
        """Write the response body content, handling chunked transfer and errors."""
    ...
    async def get(self, path: str, include_body: bool = True) -> None:
        ...
        # Handle Range requests and get file size
        size = self.get_content_size()
        range_header = self.request.headers.get("Range")
        start, end, range_status = self._handle_range_request(range_header, size)

        # If range request was invalid, response already set and we return
        if range_status == 416:
            return

        # Calculate and set Content-Length header
        content_length = self._calculate_content_length(size, start, end)
        self.set_header("Content-Length", content_length)

        # Write response body if requested
        if include_body:
            await self._write_content_body(start, end)
        else:
